...
\subsubsection{Now.js}
Now.js is a javascript library that runs on both front and back ends of the application. It is used to implement remote procedure calls (RPC) from client to server and vice-versa. It is an extra layer over Socket.io, which is a javascript library that manages the use of web sockets. 
\\
\\
The advantages of using Now.js was that it has a very simple interface of doing event-driven development, which is the use of remote procedure calls. Now.js also had some nice features. One of these is using pockets to synchronise methods and variables. A pocket is a variable space inside the client which is synchronised with the server, or vice-versa. We used this when implementing multiple concurrent games, as is explained later in this section.
\\
\\
The disadvantages of Now.js is that it is a library that is built on top of another libraries, and both libraries are quite new. We had issues getting this setup, since there are different version dependencies, but once we got it to work, we decided to stick with it.
